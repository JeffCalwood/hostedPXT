...
(function (pxsim) {
    ...
    var RefBuffer = /** @class */ (function (_super) {
        ...
        RefBuffer.prototype.toDebugString = function () {
...
(function (pxsim) {
    ...
    var FRAME_DATA_MESSAGE_CHANNEL = "messagechannel";
    var MESSAGE_SOURCE = "pxtdriver";
    var SimulatorDriver = /** @class */ (function () {
        function SimulatorDriver(container, options) {
            ...
            var messageSimulators = (_a = options) === null || _a === void 0 ? void 0 : _a.messageSimulators;
            if (messageSimulators) {
                Object.keys(messageSimulators)
                    .map(function (channel) { return messageSimulators[channel]; })
                    .forEach(function (messageSimulator) {
                    _this._allowedOrigins.push(new URL(messageSimulator.url).origin);
                    if (messageSimulator.localHostUrl)
                        _this._allowedOrigins.push(new URL(messageSimulator.localHostUrl).origin);
                });
            }
            this._allowedOrigins = pxsim.U.unique(this._allowedOrigins, function (f) { return f; });
        }
        ...
        SimulatorDriver.prototype.getSimUrl = function () {
            var simUrl = this.options.simUrl || (window.pxtConfig || {}).simUrl || location.origin + "/sim/simulator.html";
            try {
        ...
        SimulatorDriver.prototype.postMessage = function (msg, source) {
            ...
            var simUrl = pxsim.U.isLocalHost() ? "*" : this.getSimUrl();
            var broadcastmsg = msg;
            if (source && ((_a = broadcastmsg) === null || _a === void 0 ? void 0 : _a.broadcast)) {
                // if the editor is hosted in a multi-editor setting
                // don't start extra frames
                var single = !!((_b = this._currentRuntime) === null || _b === void 0 ? void 0 : _b.single);
                ...
                if (this.options.nestedEditorSim && parentWindow) {
                    ...
                    if (source !== parentWindow) {
                        var parentOrigin = this.options.parentOrigin || window.location.origin;
                    ...
                    var messageChannel_1 = msg.type === "messagepacket" && msg.channel;
                    var messageSimulator = messageChannel_1 &&
                        this.options.messageSimulators &&
                        this.options.messageSimulators[messageChannel_1];
                    // should we start an extension editor?
                    if (messageSimulator) {
                        // find a frame already running that simulator
                        var messageFrame = frames.find(function (frame) { return frame.dataset[FRAME_DATA_MESSAGE_CHANNEL] === messageChannel_1; });
                        // not found, spin a new one
                        if (!messageFrame) {
                            var useLocalHost = pxsim.U.isLocalHost() && /localhostmessagesims=1/i.test(window.location.href);
                            var url = ((useLocalHost && messageSimulator.localHostUrl) || messageSimulator.url)
                                .replace("$PARENT_ORIGIN$", encodeURIComponent(this.options.parentOrigin || ""));
                            var wrapper = this.createFrame(url);
                            this.container.appendChild(wrapper);
                            messageFrame = wrapper.firstElementChild;
                            messageFrame.dataset[FRAME_DATA_MESSAGE_CHANNEL] = messageChannel_1;
                            pxsim.U.addClass(wrapper, "simmsg");
                            pxsim.U.addClass(wrapper, "simmsg" + messageChannel_1);
                        ...
                        var mkcdFrames = frames.filter(function (frame) { return !frame.dataset[FRAME_DATA_MESSAGE_CHANNEL]; });
        ...
        SimulatorDriver.prototype.createFrame = function (url) {
            var _this = this;
            ...
            var furl = (url || this.getSimUrl()) + '#' + frame.id;
            frame.src = furl;
            frame.frameBorder = "0";
        ...
        SimulatorDriver.prototype.applyAspectRatioToFrame = function (frame, ratio) {
            ...
            var messageChannel = frame.dataset[FRAME_DATA_MESSAGE_CHANNEL];
            if (messageChannel) {
                var messageSimulatorAspectRatio = (_c = (_b = (_a = this.options) === null || _a === void 0 ? void 0 : _a.messageSimulators) === null || _b === void 0 ? void 0 : _b[messageChannel]) === null || _c === void 0 ? void 0 : _c.aspectRatio;
            ...
            if (!this.listener) {
                this.listener = function (ev) {
                    ...
                    if (pxsim.U.isLocalHost()) {
                        ...
                        if (_this._allowedOrigins.indexOf(ev.origin) < 0)
                            return;
                    }
...
(function (pxsim) {
    ...
    var EventBus = /** @class */ (function () {
        function EventBus(runtime, board, valueToArgs) {
            this.runtime = runtime;
            this.board = board;
            this.valueToArgs = valueToArgs;
            this.queues = {};
        ...
        EventBus.prototype.handleMessage = function (msg) {
    ...
    (function (AudioContextManager) {
        ...
        function getRectNoiseBuffer() {
            ...
            if (!rectNoiseBuffer) {
                var bufferSize = 131072; // must be a multiple of 4
                rectNoiseBuffer = context().createBuffer(1, bufferSize, context().sampleRate);
                var output = rectNoiseBuffer.getChannelData(0);
                var x = 0xf01ba80;
        ...
        function getCycleNoiseBuffer(bits) {
            if (!cycleNoiseBuffer[bits]) {
                ...
                var cycle_bits = [0x2df0eb47, 0xc8165a93];
                var mask_456 = [0xf, 0x1f, 0x3f];
                for (var i = 0; i < bufferSize; i += 4) {
                    var cycle = i / 4;
                    var is_on = void 0;
                    var cycle_mask = mask_456[bits - 4];
                    cycle &= cycle_mask;
        ...
        */
        ...
        function getGenerator(waveFormIdx, hz) {
            ...
            node.buffer = buffer;
            node.loop = true;
            var isFilteredNoise = waveFormIdx == 4 || (16 <= waveFormIdx && waveFormIdx <= 18);
            if (isFilteredNoise)
        ...
        function playInstructionsAsync(b) {
            ...
            var loopAsync = function () {
                ...
                var isFilteredNoise = soundWaveIdx == 4 || (16 <= soundWaveIdx && soundWaveIdx <= 18);
                ...
                if (currWave != soundWaveIdx || currFreq != freq || freq != endFreq) {
                    ...
                    if (endFreq != freq) {
                        if (ch.generator.frequency != undefined) {
                            ...
                            var bufferSamplesPerWave = isFilteredNoise ? 4 : 1024;
        ...
        function setCurrentToneGain(gain) {
            var _a;
            if ((_a = _vca) === null || _a === void 0 ? void 0 : _a.gain) {

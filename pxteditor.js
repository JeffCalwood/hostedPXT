                        ...
                        p = p.then(function () {
                            return getEditorAsync().then(function (projectView) {
                                ...
                                switch (req.action.toLowerCase()) {
                                    ...
                                    case "startactivity": {
                                        var msg_1 = data;
                                        var tutorialPath_1 = msg_1.path;
                                        var editorProjectName_1 = undefined;
                                        if (/^([jt]s|py|blocks?):/i.test(tutorialPath_1)) {
                                            if (/^py:/i.test(tutorialPath_1))
                                                editorProjectName_1 = pxt.PYTHON_PROJECT_NAME;
                                            else if (/^[jt]s:/i.test(tutorialPath_1))
                                                editorProjectName_1 = pxt.JAVASCRIPT_PROJECT_NAME;
                                            else
                                                editorProjectName_1 = pxt.BLOCKS_PROJECT_NAME;
                                            tutorialPath_1 = tutorialPath_1.substr(tutorialPath_1.indexOf(':') + 1);
                                        }
                                        return Promise.resolve()
                                            .then(function () { return projectView.startActivity(msg_1.activityType, tutorialPath_1, undefined, editorProjectName_1); });
                                    }
                                    case "importtutorial": {
                                        var load_2 = data;
                                        return Promise.resolve()
    ...
    (function (editor) {
        ...
        (function (experiments_1) {
            function key(experiment) {
                var id = (typeof experiment === "object") ? experiment.id : experiment;
            ...
            function all() {
                ...
                ].filter(function (experiment) { return ids.indexOf(experiment.id) > -1 && !(pxt.BrowserUtils.isPxtElectron() && experiment.enableOnline); });
            }
...
(function (pxt) {
    ...
    (function (shell) {
        ...
        function init() {
            ...
            var noproject = /noproject=1/i.test(window.location.href);
    ...
    (function (editor) {
        ...
        var MonacoSpriteEditor = /** @class */ (function (_super) {
            ...
            MonacoSpriteEditor.prototype.textToValue = function (text) {
                ...
                var match = pxt.parseAssetTSReference(text);
                if (match) {
                    var type = match.type, name_1 = match.name;
                    var project = pxt.react.getTilemapProject();
                    this.isAsset = true;
                    var asset = project.lookupAssetByName("image" /* Image */, name_1);
                    if (asset) {
                        return asset;
                    }
                    else {
                        var newAsset = project.createNewImage();
                        if (name_1 && !project.isNameTaken("image" /* Image */, name_1) && pxt.validateAssetName(name_1)) {
                            newAsset.meta.displayName = name_1;
                        }
                        return newAsset;
                    }
                }
    ...
    (function (editor) {
        ...
        var MonacoTilemapEditor = /** @class */ (function (_super) {
            ...
            MonacoTilemapEditor.prototype.textToValue = function (text) {
                ...
                var project = pxt.react.getTilemapProject();
                var allTiles = project.getProjectTiles(tm.data.tileset.tileWidth, true);
                tm.data.projectReferences = [];
                var _loop_1 = function (tile) {
                    if (!tm.data.tileset.tiles.some(function (t) { return t.id === tile.id; })) {
            ...
            MonacoTilemapEditor.prototype.readTilemap = function (text) {
                ...
                var id;
                if (name) {
                    var id_1 = ts.pxtc.escapeIdentifier(name);
                    proj = project.getTilemap(id_1);
                }
                if (!proj) {
                    var tileWidth = 16;
                    ...
                    var name_2 = project.createNewTilemap(id, tileWidth, 16, 16)[0];
                    proj = project.getTilemap(name_2);
                    id = name_2;
                }
                return proj;
            };
            MonacoTilemapEditor.prototype.resultToText = function (asset) {
                var project = pxt.react.getTilemapProject();
                project.pushUndo();
